...
import { adminotp, updateadmin, verifyOtpAdmin } from "../utils/APIRoutes";

const AdminDialog = ({ visible, onClose }) => {
  ...
  const handleOTP = async (e) => {
    ...
    if (otpCode) {
      ...
      if (codeString.length === 6) {
        ...
        try {
          const data = await fetch(verifyOtpAdmin, {
            method: "POST",
            headers: {
  ...
  const handleAdminChange = async (e) => {
    ...
    try {
      ...
      const updated = await fetch(updateadmin, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const UpdateResponse = await updated.json();
