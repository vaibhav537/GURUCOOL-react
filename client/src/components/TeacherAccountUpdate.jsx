...
import { checkTeacherPassword, updateTeacher } from "../utils/APIRoutes";
...
const TeacherAccountUpdate = ({ visible, onClose, teacherEmail }) => {
  ...
  const handlePasswordDialogClick = async (event) => {
    ...
    const matchPassword = await fetch(checkTeacherPassword, {
      method: "POST",
      headers: {
  ...
  const handleUpdateTeacher = async (event) => {
    ...
    try {
      const updateTeacherResult = await fetch(updateTeacher, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const updateTeacherResponse = await updateTeacherResult.json();

      if (updateTeacherResponse.success === true) {
