...
import { generateOtp, registerTeacher, verifyOtpUser } from "../utils/APIRoutes";
...
const TeacherRegister = () => {
  ...
  const handleOtpCode = async (e) => {
    ...
    if (!otp) {
      ...
      try {
        const otpResult = await fetch(verifyOtpUser, {
          method: "POST",
          headers: {
  ...
  const verifyEmail = async (event) => {
    ...
    if (!email) {
      ...
      const VerifyResult = await fetch(generateOtp, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await fetch(registerTeacher, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
              ...
              <button
                ...
                className={` ${
                  disableButton
                    ...
                    : "font-bold font-Crimson text-lg text-white bg-emerald-500 uppercase rounded  hover:bg-green-300  hover:text-green-500 transition-all duration-700 dark:text-white bg-transparent p-2 px-5 dark:bg-green-700 dark:hover:bg-green-900 cursor-pointer hover:shadow-3xl"
                }`}
